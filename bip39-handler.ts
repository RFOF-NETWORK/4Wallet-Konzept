// src/core/bip39-handler.ts
// PZQQET-Axiom: Lokale Verifizierung & Integritätsprüfung gegen BIP39-Standard

// Hier fügst du später die Liste mit den 2048 Wörtern ein (siehe Schritt 2)
...
export const verifySeedPhrase = (phrase: string): { isValid: boolean; error?: string } => {
    const words = phrase.toLowerCase().split(' ');
    
    // 1. Längenprüfung
    if (words.length !== 12 && words.length !== 24) {
        return { isValid: false, error: "Seed muss 12 oder 24 Wörter lang sein." };
    }

    // 2. Integritätsprüfung (Prüfung gegen offizielle Wortliste)
    const allWordsValid = words.every(word => BIP39_WORDLIST.includes(word));
    
    if (!allWordsValid) {
        return { isValid: false, error: "Ungültiges Wort in der Seed-Phrase gefunden." };
    }

    return { isValid: true };
};

console.log("BIP39-Handler: Integritätsprüfung für @RFOF-NETWORK aktiv.");